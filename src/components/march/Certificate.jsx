import "./Certificate.css";

export default function Certificate() {
  return (
    <section className="march-certificate">
      <div className="march-certificate-container">

        <h2>Sertifikat i profesionalna vrednost</h2>

        <p>
          Po završetku programa polaznici dobijaju sertifikat o uspešno
          završenoj obuci.
        </p>

        <p>
          Sertifikat potvrđuje stečeno znanje i može biti dodat profesionalnom
          CV-u, LinkedIn profilu...
        </p>

        <div className="certificate-highlight">
          Fokus programa je praktična primena - znanje koje odmah možete koristiti.
        </div>

      </div>
    </section>
  );
}